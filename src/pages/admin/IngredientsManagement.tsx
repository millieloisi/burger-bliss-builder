...
const IngredientsManagement = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const ingredientData = { nombre: formData.nombre, precio_extra: parseInt(formData.precio_extra), disponible: formData.disponible };
  ...
  const handleEdit = (ingredient: any) => {
    ...
    setFormData({ nombre: ingredient.nombre, precio_extra: String(ingredient.precio_extra || 0), disponible: !!ingredient.disponible });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toast.error(err.message || 'Error deleting ingredient');
    }
  };

  const resetForm = () => { setFormData({ nombre: '', precio_extra: '', disponible: true }); setEditingIngredient(null); };
...
const IngredientsManagement = () => {
  ...
  const resetForm = () => {
    setFormData({ nombre: '', precio_extra: '', disponible: true });
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) resetForm(); }}>
            ...
            <DialogContent>
              <DialogHeader><DialogTitle>{editingIngredient ? 'Edit Ingredient' : 'Create Ingredient'}</DialogTitle></DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2"><Label htmlFor="nombre">Name</Label>
                  <Input id="nombre" value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })} required /></div>
                <div className="space-y-2"><Label htmlFor="precio_extra">Extra Price</Label>
                  <Input id="precio_extra" type="number" min="0" value={formData.precio_extra} onChange={(e) => setFormData({ ...formData, precio_extra: e.target.value })} required /></div>
                <div className="flex items-center space-x-2"><input type="checkbox" id="disponible" checked={formData.disponible} onChange={(e) => setFormData({ ...formData, disponible: e.target.checked })} className="h-4 w-4"/><Label htmlFor="disponible">Available</Label></div>
                <Button type="submit" className="w-full" disabled={loading}>{loading ? 'Saving...' : editingIngredient ? 'Update Ingredient' : 'Create Ingredient'}</Button>
              </form>
            </DialogContent>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {ingredients.map((ingredient) => (
            <Card key={ingredient.id}>
              <CardHeader><CardTitle className="flex justify-between items-start"><span>{ingredient.nombre}</span><span className="text-primary font-bold">+${ingredient.precio_extra}</span></CardTitle></CardHeader>
              <CardContent>
                <p className="text-xs text-muted-foreground mb-4">{ingredient.disponible ? 'Available' : 'Unavailable'}</p>
                <div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => handleEdit(ingredient)}><Pencil className="h-4 w-4"/></Button>
                <Button variant="destructive" size="sm" onClick={() => handleDelete(ingredient.id)}><Trash2 className="h-4 w-4"/></Button></div>
              </CardContent>
            </Card>
          ))}
        </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const ingredientData = {
        ...
        precio_extra: parseInt(formData.precio_extra),
          ...
          <Dialog open={isDialogOpen} onOpenChange={(open) => {
            ...
            <DialogContent>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="precio_extra">Extra Price</Label>
                  <Input
                    id="precio_extra"
                    type="number"
                    min="0"
            ...
            <Card key={ingredient.id}>
              <CardHeader>
                <CardTitle className="flex justify-between items-start">
                  ...
                  <span className="text-primary font-bold">+${ingredient.precio_extra}</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-xs text-muted-foreground mb-4">
                  {ingredient.disponible ? 'Available' : 'Unavailable'}
                </p>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => handleEdit(ingredient)}>
                    <Pencil className="h-4 w-4" />
                  </Button>
                  <Button variant="destructive" size="sm" onClick={() => handleDelete(ingredient.id)}>
